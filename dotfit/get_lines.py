...
import dotfit
from dotfit import plot_ion_terms

datadir = Path(dotfit.__file__).parent / 'data/emission_lines/'
t = Table.read(datadir / "feti_kurucz.csv")
plot_ion_terms(t, ion="Fe II", wave=(4000, 5500), gf_min=5e-4, label_strong=0.01, eV=(0, 4), fwhm=2000, Te=10_000, figsize=(6, 4)) # fmt: skip

el.table = t
groups = [{'name': 'FeII_optical'}]
groups.append(el.to_lines('emission', 'FeII', ['b4F', 'b4P', 'a6S', 'a4G'], gf_min=1e-5, wave=[4000, 6000])) # fmt: skip
...
opt = el.get_table(wave=[3700, 7100], filter='line_ratio>1e-5')
plt.hist(np.log10(opt['line_ratio']), bins=50, range=(-10, 1))
