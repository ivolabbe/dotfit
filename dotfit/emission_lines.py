...
def _compute_boltzmann_ratios(wave_vac, Ek, gu, f, Te):
    ...
    weights = (
        (wave_vac[0] / wave_vac) ** 3
        * (f / f[0])
        * (gu / gu[0])
        * np.exp(-delta_E / (KB_EV * Te))
    )
...
def _compute_group_ratios(group, Te, Ne, verbose=False):
    ...
    if _should_use_pyneb(ion, group):
        ratios = _compute_pyneb_ratios(
            ion, group['wave_vac'], Te, Ne, tolerance=0.1, verbose=verbose
        )
...
def compute_line_ratios(
    ...
    if 'multiplet' not in tab.colnames:
        raise ValueError(
            "Table must have 'multiplet' column. Run assign_multiplets() first."
        )
    ...
    for ion in np.unique(out['ion']):
        ...
        for m in np.unique(out['multiplet'][ion_mask]):
            ...
            if verbose:
                print(
                    f"{ion} multiplet {m}: method={method}, "
                    f"ratios={ratios}, n={len(ratios)}"
                )
